...
for _ in range(1):
    ...
    t2 = datetime.now()
    print("\nVisor model time : " + str(t2 - t1) + "\n")
    VisorTimes.append(t2-t1)
    ...
    t2 = datetime.now()
    print("\nMask model time : " + str(t2 - t1) + "\n")
    MaskTimes.append(t2-t1)
    ...
    print("\nGlove model time : " + str(t2 - t1) + "\n")
    GloveTimes.append(t2-t1)
...
GownTimes = np.array(GownTimes)

visorTime = np.mean(VisorTimes)
maskTime = np.mean(MaskTimes)
gloveTime = np.mean(GloveTimes)
gownTime = np.mean(GownTimes)

print("Visor time: " + str(visorTime))
